...
public class FolderViewController {
    ...
    private void handleDeleteFolder() {
        ...
        if (user == null || folderId == null) {
            AlertUtils.showError("Error", "Invalid session or folder");
            return;
        }

        // Show confirmation dialog
        String confirmMessage = "Are you sure you want to delete the folder \"" + folderName + "\" and all its files? This action cannot be undone.";
        ...
        AlertUtils.showConfirmation(
            ...
            () -> {
                ...
                Thread deleteThread = new Thread(() -> {
                    try {
                        // Delete folder from database (which will also delete associated files)
                        boolean success = FileDAO.deleteFolder(folderId, user.getId());

                        Platform.runLater(() -> {
                            if (success) {
                                AlertUtils.showSuccess("Deleted", "Folder \"" + folderName + "\" and its files were deleted successfully.");
                                // Go back to dashboard
                                handleBack();
                            } else {
                                AlertUtils.showError("Delete Failed", "Could not delete folder. Please try again.");
                            }
                        });
                        ...
                        e.printStackTrace();
                        Platform.runLater(() ->
                            AlertUtils.showError("Delete Failed", "An error occurred: " + e.getMessage())
                        );
                    }
                });
