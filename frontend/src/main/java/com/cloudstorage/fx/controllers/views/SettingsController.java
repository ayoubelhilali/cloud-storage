...
public class SettingsController {
    ...
    private void loadStorageStats() {
        ...
        new Thread(() -> {
            try {
                ...
                // Create pie data in FIXED ORDER to match CSS color classes
                // Order: Images (0), Videos (1), Documents (2), Audio (3), Others (4)
                ObservableList<PieChart.Data> pieData = FXCollections.observableArrayList();

                // Add in specific order to match CSS default-color0, default-color1, etc.
                String[] categories = {"Images", "Videos", "Documents", "Audio", "Others"};
                for (String category : categories) {
                    Long size = typeSizes.get(category);
                    if (size != null && size > 0) {
                        pieData.add(new PieChart.Data(category, size / (1024.0 * 1024.0)));
                    }
                }
                ...
                Platform.runLater(() -> {
                    ...
                    // No need to call applyPieColors() - CSS handles it now
                });
            } catch (Exception e) { e.printStackTrace(); }
        }).start();
    }
    ...
    private void applyPieColors() {
        // CSS now handles the colors via default-color0, default-color1, etc.
        // This method is kept for potential future customizations
        if (storagePieChart == null || storagePieChart.getData() == null) return;

        // Optional: Add white borders to pie slices for better separation
        for (PieChart.Data data : storagePieChart.getData()) {
            if (data.getNode() != null) {
                data.getNode().setStyle(data.getNode().getStyle() + "; -fx-border-color: white; -fx-border-width: 2;");
            }

            // Add listener for dynamically created nodes
            data.nodeProperty().addListener((obs, oldNode, newNode) -> {
                if (newNode != null) {
                    newNode.setStyle(newNode.getStyle() + "; -fx-border-color: white; -fx-border-width: 2;");
                }
            });
        }
    }
            ...
            try {
                ...
                Platform.runLater(() -> {
                    if (success) {
                        ...
                        com.cloudstorage.util.NotificationHelper.notifyPasswordChanged(user.getId());
